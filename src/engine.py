...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, args):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if args.benchmark:
            ...
            if args.coco_path is not None and 'data/york_processed' in args.coco_path:
            ...
            elif args.coco_path is not None and 'data/wireframe_processed' in args.coco_path:
                ...
                checkpoint_path = args.output_dir+append_path+'/{}.npz'
                curr_img_id = targets[0]['image_id'].tolist()[0]
                np.savez(checkpoint_path.format(str(id_to_img[curr_img_id])),**{'lines': line, 'score':score})
        num_images +=1
