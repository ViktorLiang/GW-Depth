...
class DataLoadPreprocess(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if self._transforms is not None:
            image, targets, aux_mats = self._transforms(image, targets, aux_mats = [depth_gt, seg_gt, refl_png])
            depth_gt, seg_gt, refl_png = aux_mats
        ...
        # assert reflec_points.shape[0] != 0, 'empty reflection points:'+rhint_jsn_path+' '+str(targets['reflection_points'])
        # if reflec_points.shape[0] == 0:
        #     reflec_points = torch.rand((self.args.num_reflection_points, 2))
        #     show_labels(image, targets, points=reflec_points, need_inv=True, tittle='transed', save_dir=self.log_dir, mode=self.mode)
        # if reflec_points.shape[0] < self.args.num_reflection_points:
        #     lack_num = self.args.num_reflection_points - reflec_points.shape[0]
        #     lack_points = torch.cat([random.choice(reflec_points)[None] for _ in range(lack_num)], dim=0)
        #     reflec_points = torch.cat([reflec_points, lack_points], dim=0)
        
        del targets['reflection_points']
        ...
        return image, depth_gt, seg_gt, refl_png, reflec_points, targets
