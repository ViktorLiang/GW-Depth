...
from src.models.points.points_sample import CertainSample, PointBasedPred
...
def attention_reverse(windows_attention, window_size, H, W):
    """
    Args:
    ...
    Returns:
        x: (B, H, W, C)
    """
    B = int(windows_attention.shape[0] / (H * W / window_size / window_size))
    C = windows_attention.shape[1]
    x = windows_attention.view(windows_attention.shape[0], C, window_size, window_size, -1)
    x = windows_attention.view(B, H // window_size, W // window_size, C, window_size, window_size, -1)
    # x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1)
    x = x.permute(0, 3, 1, 4, 2, 5, 6).contiguous().view(B, C, H, W, -1)
    return x
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, x_ref=None, **kwargs):
        ...
        ref_attn_r = ref_attn_r.reshape(rB, self.num_heads, n_win, N, n_rf).permute(0, 2, 1, 3, 4).reshape(
            rB*n_win, self.num_heads, N, n_rf) # (num_windows*B, num_head, N, num_ref)
        
        ref_attn = self.softmax(ref_attn_r)
        ...
        return x, ref_attn_r, kwargs['depth_token'], kwargs['seg_token']
...
class WindowClassAttention(nn.Module):
    ...
    def forward(self, x, mask=None, x_ref=None, depth_token=None, seg_token=None, split_attention=False
    ):
        """ Forward function.
        ...
        if self.group_attention:
            ...
            if not split_attention:
                ...
                num_ref = self.args.num_ref
                ref_k = ref_k.reshape(B_, self.num_heads, num_ref, -1, C // self.num_heads)
                npnt = ref_k.shape[-2]
                ref_k_list = torch.chunk(ref_k, npnt, dim=-2)
                attn_list = []
                for ref in ref_k_list:
                    ref = ref.squeeze(-2)
                    attn_pnt = q @ ref.transpose(-2, -1)
                    attn_list.append(attn_pnt.unsqueeze(-1))
                ref_attn = torch.cat(attn_list, dim=-1).flatten(-2, -1)
            ...
            if not split_attention:
                ...
                num_ref = self.args.num_ref
                ref_v = ref_v.reshape(B_, self.num_heads, num_ref, -1, C // self.num_heads)
                npnt = ref_v.shape[-2]
                ref_v_list = torch.chunk(ref_v, npnt, dim=-2)

                ref_a = ref_attn.reshape(B_, self.num_heads, N,  num_ref, -1)
                ref_a_list = torch.chunk(ref_a, npnt, dim=-1)

                q_list = []
                for refa, refv in zip(ref_a_list, ref_v_list):
                    refa = refa.squeeze(-1)
                    refv = refv.squeeze(-2)
                    q_pnt = refa @ refv
                    q_list.append(q_pnt.unsqueeze(-1))
                q_stack = torch.cat(q_list, dim=-1)
                q_new = torch.sum(q_stack, dim=-1)
        ...
        return x, ref_attn_r, depth_token, seg_token
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, ref_coors, ref_pos=None, depth_token=None, seg_token=None, point_token=None, token_pos=None):
        ...
        if not isinstance(self.attn, OriginWindowAttention): 
            attn_windows, win_attn, depth_token, seg_token = self.attn(x_windows, mask=attn_mask, x_ref=x_ref, depth_token=depth_token, seg_token=seg_token)  # nW*B, window_size*window_size, C
        else:
        ...
        if win_attn is not None:
            win_attn = attention_reverse(win_attn, self.window_size, Hp, Wp)

        # reverse cyclic shift
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            if win_attn is not None:
                win_attn = torch.roll(win_attn, shifts=(self.shift_size, self.shift_size), dims=(2, 3))
        else:
            x = shifted_x
        ...
        if pad_r > 0 or pad_b > 0:
            x = x[:, :H, :W, :].contiguous()
            if win_attn is not None:
                win_attn = win_attn[:, :, :H, :W, :].contiguous()
        ...
        return x, win_attn, depth_token, seg_token, point_token
    ...
    def forward(self, x, H, W, ref_coors, ref_pos, depth_token=None, seg_token=None, point_token=None, token_pos=None, depth_pred=None):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x, attn_recover, depth_token, seg_token, point_token = blk(x, attn_mask, ref_coors=ref_coors, ref_pos=ref_pos, 
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x, H, W, x_down, Wh, Ww, depth_token, seg_token, point_token, ref_coors, depth_pred, attn_recover
        else:
            return x, H, W, x, H, W, depth_token, seg_token, point_token, ref_coors, depth_pred, attn_recover
...
class ReferTransformer(nn.Module):
    def __init__(self, args, feat_out_channels):
        ...
        self.depth_pred32 = nn.Sequential(nn.Linear(args.dense_trans_dim, args.class_token_dim),
        ...
        if args.depth_sample_layers[0]:
            self.certainSample1 = CertainSample(args.dense_trans_dim // 2, args.min_depth_eval, args.max_depth_eval, sample_num=args.interval_sample_num[0])
            self.point_based_pred1 = PointBasedPred(args.dense_trans_dim // 4, args.class_token_dim, pool_sizes=[16, 8, 4, 2], point_num=args.interval_sample_num[0])
        ...
        if args.depth_sample_layers[1]:
            self.certainSample2 = CertainSample(args.dense_trans_dim // 4, args.min_depth_eval, args.max_depth_eval, sample_num=args.interval_sample_num[1])
            self.point_based_pred2 = PointBasedPred(args.dense_trans_dim // 8, args.class_token_dim, pool_sizes=[16, 8, 4, 2], point_num=args.interval_sample_num[1])
        else:
        ...
        #if args.depth_sample_layers[2]:
        #    self.certainSample3 = CertainSample(args.dense_trans_dim // 8, args.min_depth_eval, args.max_depth_eval, sample_num=args.interval_sample_num[2])
        #    self.point_based_pred3 = PointBasedPred(args.dense_trans_dim // 16, args.class_token_dim, pool_sizes=[32, 16, 8, 4], point_num=args.interval_sample_num[2])
        #else:
    ...
    def forward(self, nested_top_mat, features, sample_points, sample_points_scores, layers_size=None, with_pos=True, 
        ...
        if self.args.with_line_depth:
            ...
            point_token = None
        d_enc_out, H, W, x, Wh, Ww, _, _, point_token, choosen_points, _, attn_recover = self.dense_transformer(top_mat.flatten(2).permute(0, 2, 1), H, W, choosen_points_origin, pos_emb, point_token=point_token)
        ...
        depth_pred0 = self.depth_pred32(d_enc_out).permute(0, 2, 1).reshape(B, -1, H, W)
        ...
        # saved = show_smapled_lines((choosen_points.detach().cpu() + 1) / 2, input_images.tensors.detach().cpu(), save_dir=self.args.output_dir+'/training_points_lines')
       
        
        cls_enc_out1, H1, W1, x1, Wh1, Ww1, depth_token, seg_token, _, choosen_points, _, attn_recover = self.class_transformer1(cls_enc_inp1, H_cls1, W_cls1, choosen_points, pos_emb_cls1, 
            depth_token=depth_token, seg_token=seg_token, token_pos=pos_tok_cls1)
        # show_sampled_points((choosen_points.detach().cpu() + 1) / 2, input_images.tensors.detach().cpu(), save_dir=self.args.output_dir+'/training_points_lines', save=saved)
        ...
        if self.args.depth_sample_layers[0]:
            choosen_points = self.certainSample1(depth_pred0, depth_pred1, self.args.depth_interval)
        ...
        cls_enc_out2, H2, W2, x2, Wh2, Ww2, depth_token, seg_token, _, _, _, attn_recover = self.class_transformer2(cls_enc_inp2, H_cls2, W_cls2, choosen_points, pos_emb_cls2, 
        ...
        if self.args.depth_sample_layers[0]:
            depth_pred2 = self.point_based_pred1(cls_enc_out2, depth_token, depth_pred1, choosen_points, H2, W2, pos_embedding=pos_emb_cls2)
        else:
            df2 = torch.cat([cls_enc_out2, depth_token], dim=-1)
            depth_pred2 = self.depth_pred8(df2).permute(0, 2, 1).reshape(B, -1, H2, W2)
        # height, width, with_pos=True, pos_embedding=None
        # if not self.args.with_line:
        
        if self.args.depth_sample_layers[1]:
            choosen_points = self.certainSample2(depth_pred1, depth_pred2, self.args.depth_interval)
        ...
        cls_enc_out3, H3, W3, x3, Wh3, Ww3, depth_token, seg_token, _, _, _, attn_recover = self.class_transformer3(cls_enc_inp3, H_cls3, W_cls3, choosen_points, pos_emb_cls3, 
            depth_token=depth_token, seg_token=seg_token, token_pos=pos_tok_cls3, depth_pred=depth_pred2)
        # show_sampled_points(choosen_points.cpu(), input_images.tensors.cpu())
        # if not self.args.with_line:

        if self.args.depth_sample_layers[2]:
            depth_pred3 = self.point_based_pred2(cls_enc_out3, depth_token, depth_pred2, choosen_points, H3, W3, pos_embedding=pos_emb_cls3)
        else:
            df3 = torch.cat([cls_enc_out3, depth_token], dim=-1)
            depth_pred3 = self.depth_pred4(df3).permute(0, 2, 1).reshape(B, -1, H3, W3)

        # if self.args.depth_sample_layers[2]:
        #     choosen_points = self.certainSample3(depth_pred2, depth_pred3, self.args.depth_interval)
