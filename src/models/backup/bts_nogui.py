...
class bts(nn.Module):
    def __init__(self, max_depth, feat_out_channels, num_features=512):
        ...
        self.conv1 = torch.nn.Sequential(nn.Conv2d(num_features // 16 + 1, num_features // 16, 3, 1, 1, bias=False),
                                         nn.ELU())
    ...
    def forward(self, features, focal, input_size):
        # skip0, skip1, skip2, skip3 = features[1], features[2], features[3], features[4]
        skip0, skip1, skip2 = features[0].tensors, features[1].tensors, features[2].tensors
        layer_sizes = [input_size]
        f_sizes = [(f.tensors.shape[2], f.tensors.shape[3]) for f in features[:-1]]
        layer_sizes += f_sizes

        dense_features = torch.nn.ReLU()(features[3])
        upconv5 = self.upconv5(dense_features, new_size=layer_sizes[3])  # H/16
        upconv5 = self.bn5(upconv5)

        concat5 = torch.cat([upconv5, skip2], dim=1)
        iconv5 = self.conv5(concat5)

        upconv4 = self.upconv4(iconv5, new_size=layer_sizes[2])  # H/8
        upconv4 = self.bn4(upconv4)
        concat4 = torch.cat([upconv4, skip1], dim=1)
        ...
        # depth_8x8 = F.interpolate(depth_8x8.unsqueeze(1), size=layer_sizes[0], mode='nearest') # resize to origin image size
        ...
        # depth_8x8_scaled_ds = F.interpolate(depth_8x8_scaled, size=layer_sizes[1], mode='nearest')

        upconv3 = self.upconv3(daspp_feat, new_size=layer_sizes[1])  # H/4
        upconv3 = self.bn3(upconv3)
        # concat3 = torch.cat([upconv3, skip0, depth_8x8_scaled_ds], dim=1)
        concat3 = torch.cat([upconv3, skip0], dim=1)
        ...
        # depth_4x4 = F.interpolate(depth_4x4.unsqueeze(1), size=layer_sizes[0], mode='nearest') # resize to origin image size
        ...
        # h_half, w_half = depth_4x4_scaled_ds.shape[-2:]
        upconv2 = self.upconv2(iconv3, new_size=(layer_sizes[1][0]*2, layer_sizes[1][1]*2))  # H/2
        upconv2 = self.bn2(upconv2)
        # concat2 = torch.cat([upconv2, depth_4x4_scaled_ds], dim=1)
        iconv2 = self.conv2(upconv2)
        ...
        # depth_2x2 = F.interpolate(depth_2x2.unsqueeze(1), size=layer_sizes[0], mode='nearest') # resize to origin image size
        # depth_2x2_scaled = depth_2x2 / self.max_depth

        upconv1 = self.upconv1(iconv2, new_size=layer_sizes[0])
        reduc1x1 = self.reduc1x1(upconv1)
        concat1 = torch.cat([upconv1, reduc1x1], dim=1)

        iconv1 = self.conv1(concat1)
        final_dense = self.max_depth * self.get_depth(iconv1)
        depth_pred = final_dense[:,0:1]
        seg_pred = final_dense[:,1:]
        # if self.params.dataset == 'kitti':
        #     final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873

        # return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1, depth_pred, seg_pred
        return None, None, None, reduc1x1, depth_pred, seg_pred
...
def build_depth_decoder(args):
    return bts(args.max_depth, [256, 512, 1024, 512], num_features=512)
