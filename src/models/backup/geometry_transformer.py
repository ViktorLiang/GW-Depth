...
from src.models.geometry_utils import TokenFuse, NonLocalPlannarGuidance, sample_by_centers
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., size_ratio=0.25, args=None):
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_attn=WindowAttention, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, size_ratio=0.25, token_fuse=False, num_points=None, args=None):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = window_attn(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, args=args)
    ...
    def forward(self, x, mask_matrix, ref_coors, ref_pos=None, depth_token=None, seg_token=None, token_pos=None):
        """ Forward function.
        ...
        if depth_token is not None and seg_token is not None:
            ...
            if self.token_fuse:
                ...
                depth_token = self.token_relation(r_seg_token.permute(0,3,1,2), r_depth_token.permute(0,3,1,2), 
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                token_fuse=token_fuse if i == depth - 1 else False,
    ...
    def forward(self, x, H, W, ref_coors, ref_pos, depth_token=None, seg_token=None, token_pos=None, depth_pred=None):
        """ Forward function.
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x, depth_token, seg_token = blk(x, attn_mask, ref_coors=ref_coors, ref_pos=ref_pos, 
                                depth_token=depth_token, seg_token=seg_token, token_pos=token_pos)
        if self.class_pred:
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x, H, W, x_down, Wh, Ww, depth_token, seg_token, ref_coors, depth_pred
        else:
            return x, H, W, x, H, W, depth_token, seg_token, ref_coors, depth_pred
...
class ReferTransformer(nn.Module):
    def __init__(self, args, feat_out_channels):
        ...
        if args.with_line:
            self.dense_transformer = BasicLayer(dim=args.dense_trans_dim, depth=args.dense_trans_layers[0], num_heads=args.dense_trans_heads, \
                window_attn=WindowAttention, window_size=7, mlp_ratio=2, size_ratio=1/32, args=args)
        else:
            self.dense_transformer = BasicLayer(dim=args.dense_trans_dim, depth=args.dense_trans_layers[0], num_heads=args.dense_trans_heads, \
        ...
        self.class_transformer1 = BasicLayer(dim=args.dense_trans_dim // 2, depth=args.class_trans_layers[0], num_heads=args.dense_trans_heads, \
            ...
            token_fuse=args.class_tokenfuse_layers[0], 
        ...
        self.class_transformer2 = BasicLayer(dim=args.dense_trans_dim // 4, depth=args.class_trans_layers[1], num_heads=args.dense_trans_heads, \
            ...
            token_fuse=args.class_tokenfuse_layers[1], 
        ...
        self.class_transformer3 = BasicLayer(dim=args.dense_trans_dim // 8, depth=args.class_trans_layers[2], num_heads=args.dense_trans_heads, \
            ...
            token_fuse=args.class_tokenfuse_layers[2], 
    ...
    def forward(self, nested_top_mat, features, sample_points, sample_points_scores, layers_size=None, with_pos=True, 
        ...
        if self.args.with_line and not self.args.with_dense_center:
            choosen_points = choosen_points[:, :, :2]
        
        pos_emb = self.pos_enc(nested_top_mat) if with_pos else None
        d_enc_out, H, W, x, Wh, Ww, _, _, choosen_points, _ = self.dense_transformer(top_mat.flatten(2).permute(0, 2, 1), H, W, choosen_points, pos_emb)
        ...
        cls_enc_out1, H1, W1, x1, Wh1, Ww1, depth_token, seg_token, choosen_points, depth_pred1 = self.class_transformer1(cls_enc_inp1, H_cls1, W_cls1, choosen_points, pos_emb_cls1, 
        ...
        cls_enc_out2, H2, W2, x2, Wh2, Ww2, depth_token, seg_token, choosen_points, depth_pred2 = self.class_transformer2(cls_enc_inp2, H_cls2, W_cls2, choosen_points, pos_emb_cls2, 
        ...
        cls_enc_out3, H3, W3, x3, Wh3, Ww3, depth_token, seg_token, choosen_points, depth_pred3 = self.class_transformer3(cls_enc_inp3, H_cls3, W_cls3, choosen_points, pos_emb_cls3, 
