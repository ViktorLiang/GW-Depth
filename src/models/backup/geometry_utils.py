...
class upconv(nn.Module):
    def __init__(self, in_channels, out_channels, ratio=0):
    ...
    def forward(self, x, size=None):
        if size is None and self.ratio > 0:
            up_x = F.interpolate(x, scale_factor=self.ratio, mode='nearest')
        elif size is not None:
            up_x = F.interpolate(x, size=size, mode='nearest')
        else:
            up_x = x
        out = self.conv(up_x)
        out = self.elu(out)
...
class NonLocalPlannarGuidance(nn.Module):
    def __init__(self, backbone_dim=128, num_points=50, pre_pred=False, args=None):
        super().__init__()
        ...
        if pre_pred:
            self.depth_pred = nn.Linear(self.class_dim, out_features=1)
        else:
            self.pre_depth_upsample = upconv(1, 1)
        
        self.depth_filter = nn.Sequential(
            ConvA(1, 4, kernel_size=3, padding=1),
            ConvA(4, 16, kernel_size=3, padding=1),
            ConvA(16, 4, kernel_size=3, padding=1),
            ConvA(4, 1, kernel_size=3, padding=1),
        )
    ...
    def forward(self, backbone_x, seg_token, depth_token, refer_coords, token_pos, depth_pred=None,  with_pos=True):
        ...
        if self.pre_pred:
            depth_pred = self.depth_pred(depth_feats)
            depth_pred = depth_pred.permute(0, 2, 1).reshape(B, -1, H, W)
        else:
        ...
        pnt_global = class_v @ class_pnt
        pnt_global_attn = self.softmax(pnt_global)

        new_depth = pnt_global_attn @ depth_pnt.permute(0, 2, 1)
        new_depth = new_depth.permute(0, 2, 1).reshape(B, 1, H, W)
        new_depth = self.depth_filter(F.layer_norm(new_depth, [H, W])) + depth_pred
        new_depth = new_depth.sigmoid()
        return new_depth, pnt_global_attn
...
class PointGuidedTokenFuse(nn.Module):
    ...
    def forward(self, backbone_x, seg_token, depth_token, refer_coords, token_pos, with_pos=True):
        ...
        depth_token = dtx.permute(0, 2, 1).reshape(B, C, H, W).contiguous()
        refer_depth = F.grid_sample(depth_token, refer_coords, mode='nearest') #(B, C, n_plane, n_point)
        if with_pos:
            samp_pos = F.grid_sample(token_pos, refer_coords, mode='nearest')
            refer_depth = refer_depth + samp_pos
