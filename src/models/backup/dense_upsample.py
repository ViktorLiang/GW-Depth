...
class DensePrediction(nn.Module):
    def __init__(self, max_depth, feat_out_channels, num_features=256, args=None):
        ...
        self.norm_depth = nn.LayerNorm(args.class_token_dim)
        ...
        self.norm_seg = nn.LayerNorm(args.class_token_dim)
    ...
    def forward(self, decoder_top_feat, decoder_depth_pred, depth_token, seg_token, input_size):
        ...
        upconv1 = self.norm_depth(self.upconv1_depth(detph_feats).permute(0, 2, 3, 1))  # H/2
        conv1d = self.conv1_depth(upconv1.permute(0, 3, 1, 2))
        ...
        upconv1_seg = self.norm_seg(self.upconv1_seg(seg_feats).permute(0, 2, 3, 1))  # H/2
        conv1s = self.conv1_seg(upconv1_seg.permute(0, 3, 1, 2))
...
def build_depth_decoder(args):
    return DensePrediction(args.max_depth, [64, 128, 256, 512], num_features=64, args=args)
